...
这是一个 Codex skill，基于 Karpathy llm-wiki 方法论构建个人知识库系统。核心理念：知识"一次编译，持续维护"，而非每次查询时重新推导。
...
```bash
# 安装到 Codex
git clone <repo> ~/.Codex/skills/llm-wiki && bash ~/.Codex/skills/llm-wiki/setup.sh
```

`setup.sh` 做了什么：
- 将 `deps/` 中的 3 个依赖 skill 复制到 `~/.Codex/skills/`
